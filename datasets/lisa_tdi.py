...
def gb_tdi_signal(
    ...
    *,
    ...
    """Time-domain TDI-X galactic-binary signal via jaxGB (matched convention).

    Args:
        n: Number of samples (``T = n * dt`` is the jaxGB observation time).
        dt: Sampling interval.
        gb_params: Dict with keys ``f0, fdot, A, ra, dec, psi, iota, phi0``.
        tdi_gen: TDI generation (use ``2.0`` to match ``XYZ2`` noise).
        channel: 0/1/2 for X/Y/Z.
    """
    _require_lisa()
    import jax
    ...
    tdi = gb.get_tdi(params, tdi_generation=tdi_gen, tdi_combination="XYZ")
