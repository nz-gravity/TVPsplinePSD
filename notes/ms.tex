...
Let $ \tilde{h}(\lambda|\theta))$ be the frequency representation of the GW signal.
\begin{definition}[Dynamic Whittle likelihood]
The \emph{dynamic Whittle likelihood} based on the moving periodogram ordinates is defined by
\begin{equation}
...
\frac{1}{f(t/T, \lambda_{\mathrm{mod}(t)})}
\exp\!\left(
-\frac{(\tilde{X}_t(\lambda_{\mathrm{mod}(t)}) - \tilde{h}(\lambda_{\mathrm{mod}(t)})|\theta))^2}{f(t/T, \lambda_{\mathrm{mod}(t)})}
\right).
%\label{eq:dynamic_whittle}
\end{equation}
\end{definition}
