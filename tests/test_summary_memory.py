...
def test_chunked_intervals_equal_full_reconstruction():
    ...
    bt = rng.normal(size=(7, 3))
    bf = rng.normal(size=(11, 4))
    draws = rng.normal(size=(23, 3, 4))
    g = rng.normal(size=(23, 4))
    ...
    np.testing.assert_allclose(
        [lo, hi], np.percentile(full, [5, 95], axis=0), atol=1e-14
    )
    ...
    np.testing.assert_allclose(
        [lo, hi], np.percentile(total, [5, 95], axis=0), atol=1e-14
    )
